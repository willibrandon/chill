...
func daemonCompatibility(s *Status, client string) (string, string) {
	if s == nil {
		return "not-running", "no daemon running; the next playback command starts one"
	}
	upgrade, err := daemonNeedsUpgrade(*s, client)
	if err != nil {
	...
	if !semver.IsValid(client) || !semver.IsValid(s.Version) || strings.Contains(client, "+dirty") || strings.Contains(s.Version, "+dirty") {
		return "unverifiable", "development/unversioned build; freshness cannot be verified; use chill --stop then chill to restart with this executable"
	}
	if semver.Compare(s.Version, client) > 0 {
